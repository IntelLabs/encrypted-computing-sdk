...
#ifndef P_ISA_TOOLS_DATA_FORMATS_OPENFHE_TRACER_HERACLESTRACER_H_
#define P_ISA_TOOLS_DATA_FORMATS_OPENFHE_TRACER_HERACLESTRACER_H_
...
public:
    ...
    // TODO: refactor - move to tracer base class
    ...
    {
        auto tracer = StartFunctionTrace(func);
...
private:
    ...
    /// Note: the match is on the PREFIX of the instruction name,
    ...
    /// WARNING: This means that InPlace versions are also matched, e.g.,
    /// LeveledSHERNS::EvalAdd will also match LeveledSHERNS::EvalAddInPlace!
    const std::unordered_set<std::string> no_emit_prefixes = {
        // High-level Multiplication Wrappers
        "CryptoContext::EvalMult",
        "LeveledSHERNS::EvalMult",
        "LeveledSHECKKSRNS::EvalMult",
        "LeveledSHERNS::AdjustForMultInPlace",
        // High-level Addition // SubtractionWrappers
        "CryptoContext::EvalAdd",
        "LeveledSHERNS::EvalAdd",
        "LeveledSHECKKSRNS::EvalAdd",
        "CryptoContext::EvalSub",
        "LeveledSHERNS::EvalSub",
        "LeveledSHECKKSRNS::EvalSub",
        "LeveledSHERNS::AdjustForAddOrSubInPlace",
    };
    ...
    const std::unordered_map<std::string, std::string> op_name_map = {
        // TODO: unify the naming here (e.g., get rid of <Element> and 1/2 on arg types)
        // Addition
        { "LeveledSHEBase<Element>::EvalAddCore(ciphertext, ciphertext)", "add" },
        { "LeveledSHEBase<Element>::EvalAddCore(ciphertext, plaintext)", "add_plain" },
        { "LeveledSHEBase<Element>::EvalAddCoreInPlace(ciphertext, ciphertext)", "add" },
        { "LeveledSHEBase<Element>::EvalAddCoreInPlace(ciphertext, plaintext)", "add_plain" },
        // Subtraction
        { "LeveledSHEBase<Element>::EvalSubCore(ciphertext, ciphertext)", "sub" },
        { "LeveledSHEBase<Element>::EvalSubCore(ciphertext, plaintext)", "sub_plain" },
        { "LeveledSHEBase<Element>::EvalSubCoreInPlace(ciphertext, ciphertext)", "sub" },
        { "LeveledSHEBase<Element>::EvalSubCoreInPlace(ciphertext, plaintext)", "sub_plain" },
        // Multiplication
        { "LeveledSHEBase<Element>::EvalMultCore(ciphertext, ciphertext)", "mul" },
        { "LeveledSHEBase<Element>::EvalMultCore(ciphertext, plaintext)", "mul_plain" },
        { "LeveledSHEBase<Element>::EvalMultCoreInPlace(ciphertext, ciphertext)", "mul" },
        { "LeveledSHEBase<Element>::EvalMultCoreInPlace(ciphertext, plaintext)", "mul_plain" },
        { "LeveledSHECKKSRNS::EvalMultCoreInPlace(ciphertext, double)", "mul_scalar" },
        // Modulus Reduction
        // TODO: NOT YET SUPPORTED BY KERNGEN/PROGRAM MAPPER, so replacing with rescale for now (WRONG!)
        { "LeveledSHECKKSRNS::ModReduceInternalInPlace", "rescale" },
        { "LeveledSHECKKSRNS::EvalMultCoreInPlace(ciphertext, ciphertext)", "mul" },
        // TODO: Relin, Bootstrap, etc
        // Rotation
        // TODO: I think we actually want to trace LOWER LEVEL functions here,
        // after OpenFHE has accounted for the rotation keys we have"
        { "CryptoContext::EvalRotate", "rotate" },
    };
...
#endif // P_ISA_TOOLS_DATA_FORMATS_OPENFHE_TRACER_HERACLESTRACER_H_
