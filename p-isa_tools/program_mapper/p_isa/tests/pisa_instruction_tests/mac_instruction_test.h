...
class macInstructionTest : public PisaInstructionTest
{
public:
    static std::string operationName() { return "mac_instruction"; }
...
public:
    void constructTest() override
    {
        ...
        std::vector<std::string> input_regs  = { { "input1" }, { "input2" } };

        auto mac_instr = new pisa::instruction::Mac(std::stoi(configuration["Poly_mod_log2"]),
        ...
        instruction_trace.push_back(mac_instr);
