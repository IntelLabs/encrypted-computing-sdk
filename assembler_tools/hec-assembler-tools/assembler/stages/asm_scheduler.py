...
# FUTURE:
# - Analyze about adding instruction window to dependency graph creation.
# - Analyze about adding terms to priority that will prioritize P-ISA instructions over all others as tie-breaker
...
class Simulation:
    ...
    def addXInstrBackIntoPipeline(self, xinstr: object):
        ...
        if not isinstance(xinstr, xinst.XStore):
            for v in xinstr.sources + xinstr.dests:
                if isinstance(v, Variable) and v.name in self.live_vars and xinstr in self.live_vars[v.name]:
    ...
    def addXInstrToTopoSort(self, xinstr_id: tuple):
        ...
        for idx, topo_instr_id in enumerate(self.topo_sort):
            ...
            elif topo_instr_id in self.dependency_graph and self.dependency_graph.in_degree(
                topo_instr_id
            ) >= self.dependency_graph.in_degree(xinstr_id):
                target_idx = idx
                break
    ...
    def addDependency(self, new_dependency_instr, original_instr):
        """
        ...
        all_vars = set(
            v
            for v in new_dependency_instr.sources + new_dependency_instr.dests
    ...
    def addExtraXStoreDependencies(self, original_instr, xstore_instr, new_var):
        ...
        added to the topo_sort.
        Parameters:
            new_var: Variable waiting for eviction to occur.
    ...
    def addLiveVar(self, var_name: str, instr):
        """
        Adds a live variable to the current bundle.
    ...
    def addUsedVar(self, var_name: str, instr):
        """
        Removes a used variable from the current bundle.
    ...
    def canSchedulerShuffle(self, xinstr) -> CycleType:
        ...
        if (
            xinstr.cycle_ready.bundle <= self.current_cycle.bundle
            ...
            assert self.current_cycle.bundle >= last_rshuffle_cycle.bundle, "Last scheduled rshuffle cannot be in the future."
            ...
            if self.current_cycle.bundle == last_rshuffle_cycle.bundle:
                ...
                if (isinstance(xinstr, xinst.rShuffle) and isinstance(self.last_xrshuffle, xinst.rShuffle)) or (
                    isinstance(xinstr, xinst.irShuffle) and isinstance(self.last_xrshuffle, xinst.irShuffle)
                ):
                    ...
                    if cycle_delta < self.last_xrshuffle.SpecialLatencyMax:
                        ...
                        retval = CycleType(bundle=self.current_cycle.bundle, cycle=last_rshuffle_cycle.cycle + cycle_delta)
                else:
    ...
    def canSchedulerShuffleType(self, xinstr) -> bool:
        ...
        if isinstance(xinstr, (xinst.rShuffle, xinst.irShuffle)):
            ...
            if self.bundle_current_rshuffle_type[0] is not None and not isinstance(xinstr, self.bundle_current_rshuffle_type[0]) and retval:
                ...
                retval = next(
                    (
                        False
                        for _, inv_rshuffle in self.priority_queue
    ...
    def canScheduleArithmeticXInstr(self, xinstr: xinst.XInstruction) -> bool:
        ...
        if xinstr.res is not None:
            ...
            assert (
                self.bundle_current_ones_segment == self.bundle_current_twid_segment
            ), "Current Ones and Twiddle metadata segments are not synchronized."
            assert (
                self.bundle_needed_ones_segment == self.bundle_needed_twid_segment
            ...
            if self.bundle_current_ones_segment != -1 and self.bundle_current_ones_segment != xinstr_required_segment and retval:
                ...
                retval = next(
                    (
                        False
                        for _, other_xinstr in self.priority_queue
            ...
            assert (
                ...
                or xinstr.cycle_ready.bundle > self.current_cycle.bundle
                or self.bundle_needed_ones_segment == -1
                or xinstr_required_segment == self.bundle_needed_ones_segment
    ...
    def findNextInstructionToSchedule(self) -> object:
        ...
        if self.priority_queue:
            while retval is None and self.priority_queue.peek()[1].cycle_ready.bundle <= self.current_cycle.bundle:
                ...
                while retval is None:
                    ...
                    if p_inst.cycle_ready.bundle < self.current_cycle.bundle:
                        ...
                        p_inst.cycle_ready = CycleType(bundle=self.current_cycle.bundle, cycle=0)
                    # Check found instruction has correct priority
                    if p_inst.cycle_ready == priority:
                        ...
                        if hasBankWriteConflict(p_inst, self):
                            # Write cycle conflict found, so, update found instruction cycle ready
                            new_cycle_ready = CycleType(
                                bundle=self.current_cycle.bundle, cycle=max(p_inst.cycle_ready.cycle, self.current_cycle.cycle) + 1
                            )
                ...
                if not self.canSchedulerShuffleType(retval):
                    ...
                    retval.cycle_ready = CycleType(bundle=self.current_cycle.bundle + 1, cycle=0)
    ...
    def flushBundle(self):
        ...
        bundle_last_xstore = next(
            (
                self.xinsts_bundle[idx]
                for idx in reversed(range(len(self.xinsts_bundle)))
        ...
        bundle_latency_from_last_xstore = (
            (bundle_latency - bundle_last_xstore.schedule_timing.cycle.cycle) if bundle_last_xstore else bundle_latency
        )
        ...
        for _ in range(self.max_bundle_size - len(self.xinsts_bundle)):
            ...
            b_scheduled = scheduleXNOP(
                instr,
                1,  # Idle cycles
                self,
                force_nop=True,
            )  # We want nop to be added regardless of last in bundle
            assert b_scheduled
        ...
        ifetch = cinst.IFetch(
            self.xinsts_bundle[0].id[1],  # ID of first instruction in bundle, just for book-keeping
        ...
        if len(self.xinsts) > 0:
            ...
            cq_throughput = 0
            while idx >= 0 and not isinstance(self.cinsts[idx], (cinst.IFetch, cinst.CStore)):
                cq_throughput += self.cinsts[idx].throughput
                idx -= 1

            # Added ifetch latency to avoid timing errors when bundles are short or empty
            idle_c_cycles = self.xinsts[-1].latency_from_xstore - cq_throughput + ifetch.latency
            if idle_c_cycles > 0:
        ...
        if self.bundle_needed_rshuffle_type is not None and self.bundle_current_rshuffle_type[0] != self.bundle_needed_rshuffle_type:
        ...
        if self.bundle_needed_twid_segment >= 0 and self.bundle_current_twid_segment != self.bundle_needed_twid_segment:
        ...
        if self.bundle_needed_ones_segment >= 0 and self.bundle_current_ones_segment != self.bundle_needed_ones_segment:
        ...
        self.xinsts.append(
            BundleData(xinsts=self.xinsts_bundle, latency=bundle_latency, latency_from_xstore=bundle_latency_from_last_xstore)
        )
        ...
        for idx, cstore_instr in enumerate(self.post_bundle_cinsts):
            ...
            if variable.name in self.mem_model.output_variables and not variable.accessed_by_xinsts:
                ...
                msyncc = minst.MSyncc(cstore_instr.id[0], cstore_instr)
                ...
                mstore = minst.MStore(
                    cstore_instr.id[0], [variable], self.mem_model, dest_hbm_addr, comment=(" id: {} - flushing").format(cstore_instr.id)
                )
        ...
        for idx in range(len(self.mem_model.register_banks)):
            ...
            for reg in bank:
                if isinstance(reg.contained_variable, DummyVariable) and reg.contained_variable.tag < self.current_cycle.bundle:
    ...
    def flushOutputVariableFromRegister(self, variable, xinstr=None) -> bool:
        """
        Flushes an output variable from the register.
        ...
        if variable.register_dirty:
            ...
            if retval:
                xstore = _createXStore(xinstr.id[0], dest_spad_addr, variable, None, " flushing output", self)
                self.addDependency(xstore, None)
                # Add to topo_sort
                self.addXInstrToTopoSort(xstore.id)
        return retval

    def generateKeyMaterial(self, instr_id: int, variable: Variable, register: Register, dep_id=None) -> int:
        """
        Generates key material for the specified variable.
        ...
        if retval == 1:
            ...
            if key_idx != self.last_keygen_index + 1:
                raise RuntimeError(
                    ('Keygen variable "{}" generation out of order. ' "Expected key index {}, but received {} for seed {}.").format(
                        variable.name, self.last_keygen_index + 1, key_idx, self.bundle_current_kgseed
                    )
                )
            ...
            cnop = cinst.CNop(instr_id, kg_load.latency, comment="kg_load {} wait period".format(kg_load.id))
            cnop.schedule(self.current_cycle, len(self.cinsts) + 1)
            self.cinsts.append(cnop)
    ...
    def loadrShuffleRoutingTable(self, rshuffle_data_type_name: str):
        """
        Queues CInstructions needed to load the `rshuffle` routing table into CE.
        ...
        if rshuffle_data_type_name == xinst.rShuffle.RSHUFFLE_DATA_TYPE:
            ...
            raise ValueError(
                ('`rshuffle_data_type_name`: invalid value "{}". Expected one of {}.').format(
                    rshuffle_data_type_name, {xinst.rShuffle.RSHUFFLE_DATA_TYPE, xinst.irShuffle.RSHUFFLE_DATA_TYPE}
                )
            )
        ...
        if aux_table_name and routing_table_name:
            ...
            spad_map[aux_table_name] = (self.mem_model.variables[aux_table_name], aux_table_target)
            spad_map[routing_table_name] = (self.mem_model.variables[routing_table_name], routing_table_target)
    ...
    def loadBOnesMetadata(self, spad_addr_offset: int, ones_metadata_segment: int) -> int:
        """
        Queues MInstructions and CInstructions needed to load the Ones metadata.
        ...
        if ones_metadata_segment < 0 or ones_metadata_segment >= len(self.mem_model.meta_ones_vars_segments):
            raise IndexError(
                (
                    "`twid_metadata_segment`: requested segment index {}, but there are only {} "
                    "segments of ones metadata available for up to {} residuals."
                ).format(
                    ones_metadata_segment,
        ...
        if meta_ones_vars and len(meta_ones_vars) != constants.MemoryModel.NUM_ONES_META_REGISTERS:
            raise RuntimeError(
                "Required {} twiddle metadata variables per segment, but {} received.".format(
                    constants.MemoryModel.NUM_ONES_META_REGISTERS, len(meta_ones_vars)
                )
            )
        ...
        for meta_ones_var_name in meta_ones_vars:
            ...
            self.queueMLoad(
                ...
                target_spad_addr,
                variable,
                comment="loading ones metadata for residuals [{}, {})".format(
                    ones_metadata_segment * constants.MemoryModel.MAX_RESIDUALS,
    ...
    def loadTwiddleMetadata(self, spad_addr_offset: int, twid_metadata_segment: int):
        """
        Queues MInstructions and CInstructions needed to load the Twiddle factor generation metadata.
        ...
        if twid_metadata_segment < 0 or twid_metadata_segment >= len(self.mem_model.meta_twiddle_vars_segments):
            raise IndexError(
                (
                    "`twid_metadata_segment`: requested segment index {}, but there are only {} "
                    "segments of twiddle metadata available for up to {} residuals."
                ).format(
                    twid_metadata_segment,
        ...
        if meta_twiddle_vars and len(meta_twiddle_vars) != self.mem_model.MAX_TWIDDLE_META_VARS_PER_SEGMENT:
            raise RuntimeError(
                "Required {} twiddle metadata variables per segment, but {} received.".format(
                    self.mem_model.MAX_TWIDDLE_META_VARS_PER_SEGMENT, len(meta_twiddle_vars)
                )
            )
        ...
        for meta_twiddle_var_name in meta_twiddle_vars:
            ...
            self.queueMLoad(
                ...
                target_spad_addr,
                variable,
                comment="loading twid metadata for residuals [{}, {})".format(
                    twid_metadata_segment * constants.MemoryModel.MAX_RESIDUALS,
    ...
    def loadKeygenSeedMetadata(self, spad_addr_offset: int, kgseed_idx: int) -> int:
        """
        Queues MInstructions and CInstructions needed to load a new keygen seed.
        ...
        if (
            ...
            raise IndexError(
                "`kgseed_idx` must index in the range [0, {}), but {} received".format(
                    len(self.mem_model.meta_keygen_seed_vars) * constants.MemoryModel.NUM_BLOCKS_PER_KGSEED_META_WORD, kgseed_idx
                )
            )
        ...
        if kgseed_idx != self.bundle_current_kgseed:
            ...
            # Load variable into SPAD
            self.queueMLoad(
                0, target_spad_addr, seed_variable, comment="loading keygen seed ({}, block = {})".format(seed_word_idx, seed_word_block)
            )
            spad_addr += 1
            ...
            kg_seed = cinst.KGSeed(len(self.cinsts), seed_word_block, seed_variable, self.mem_model)
    ...
    def prepareShuffleMetadata(self, spad_addr_offset: int) -> int:
        """
        Queues MInstructions needed to load the `rshuffle` metadata into SPAD.
        ...
        # Load HBM -> SPAD
        if self.mem_model.meta_ntt_aux_table and self.mem_model.meta_ntt_routing_table:
        ...
        if self.mem_model.meta_intt_aux_table and self.mem_model.meta_intt_routing_table:
    ...
    def queueCSyncmLoad(self, instr_id: int, source_spad_addr: int):
        """
    ...
    def queueMSynccLoad(self, instr_id: int, target_spad_addr: int):
        """
    ...
    def updateSchedule(self, instr) -> bool:
        ...
        if isinstance(instr, xinst.XStore):
            ...
            cstore = cinst.CStore(instr.id[0], self.mem_model, comment=instr.comment)
            ...
            if self.pre_bundle_csync_minstr[0] <= last_mstore[0] and last_mstore[1] is not None:
        ...
        if isinstance(instr, (xinst.rShuffle, xinst.irShuffle)):
            ...
            write_cycle = XWriteCycleTrack(
                cycle=CycleType(bundle=scheduled_cycle.bundle, cycle=scheduled_cycle.cycle + instr.latency - 1),
            ...
            if self.bundle_needed_rshuffle_type is None:
                ...
                raise RuntimeError(
                    "Attempted to schedule {} in bundle that only allows {}.".format(instr, self.bundle_needed_rshuffle_type)
                )
        ...
        if instr.res is not None:
            ...
            if self.bundle_needed_ones_segment == -1:
                ...
                raise RuntimeError(
                    (
                        'Attempted to schedule XInstruction "{}", residual = {}, '
                        "in bundle {} that only allows residuals in range [{}, {})."
                    ).format(
                        str(instr),
                        instr.res,
                        self.current_cycle.bundle,
...
def _createXStore(
    instr_id: int, dest_spad_addr: int, evict_variable: Variable, new_variable: Variable, comment: str, simulation: Simulation
) -> object:
    """
    ...
    xstore = (
        XStoreAssign(instr_id, [evict_variable], simulation.mem_model, new_variable, dest_spad_addr=dest_spad_addr, comment=comment)
        if new_variable
        else xinst.XStore(instr_id, [evict_variable], simulation.mem_model, dest_spad_addr=dest_spad_addr, comment=comment)
    )
...
def __flushVariableFromRegisterFile(
    instr, dest_spad_addr: int, evict_variable: Variable, new_variable: Variable, simulation: Simulation
) -> object:
    """
...
def findRegister(
    instr, bank_idx: int, simulation: Simulation, override_replacement_policy: str = None, dest_var: Variable = None
) -> object:
    """
    ...
    #                   no need to flush register, just evict variable since it has not been written to.
...
def hasBankWriteConflictGeneral(ready_cycle: CycleType, latency: int, banks, simulation: Simulation) -> bool:
    ...
    if ready_cycle.bundle <= simulation.current_cycle.bundle:  # Instruction has no conflicts if it is on a later bundle
        instr_write_cycle = XWriteCycleTrack(
            cycle=CycleType(
                bundle=simulation.current_cycle.bundle, cycle=max(ready_cycle.cycle, simulation.current_cycle.cycle) + latency - 1
            ),
            banks=set(banks),
        )
...
def prepareInstruction(original_xinstr, simulation: Simulation) -> int:
            ...
            if isinstance(src_var, Variable):
                if src_var.name in simulation.live_outs:
                    ...
                    if not src_var.register:
                        ...
                        b_generated_keygen_var = (
                            not simulation.mem_model.isVarInMem(src_var.name) and src_var.name in simulation.mem_model.keygen_variables
                        )
                        ...
                        if not b_generated_keygen_var:
                            ...
                            if src_var.spad_address < 0:
                                assert (
                                    src_var.name not in simulation.mem_model.store_buffer
                                ), f'Attempting to load from HBM: "{src_var.name}"; already in transit in SPAD store buffer.'
                                if not loadVariableHBMToSPAD(original_xinstr, src_var, simulation):
                                    # Could not find location in SPAD, move to next bundle
                                    retval = 0

                        if retval != 0:
                            retval, new_instr_or_reg = findRegister(
                                original_xinstr, 0, simulation, override_replacement_policy=""
                            )  # No replacement policy for bank 0
                            ...
                            if retval == 1:
                                ...
                                if b_generated_keygen_var:
                                    ...
                                    cload = cinst.CLoad(
                                        original_xinstr.id[0],
                                        new_instr_or_reg,
                                        [src_var],
                                        simulation.mem_model,
                                        comment="dep id: {}".format(original_xinstr.id),
                                    )
                    ...
                    if retval == 1:
                        if src_var.register.bank.bank_index == 0:
                            ...
                            if retval == 1:
                                ...
                                xmove = xinst.Move(
                                    original_xinstr.id[0], new_instr_or_reg, [src_var], dummy_var=simulation.bundle_dummy_var
                                )
                    ...
                    if retval == 1:
                        if src_var.register.bank.bank_index != src_var.suggested_bank:
                            raise RuntimeError(
                                "Variable `{}` is in register `{}`, which is not in suggested bank {}.".format(
                                    src_var.name, src_var.register.name, src_var.suggested_bank
                                )
                            )
                        if b_generated_keygen_var:
                ...
                if dst_var.name in simulation.live_outs:
                    ...
                    if retval == 1:
                        if dst_var.register.bank.bank_index != dst_var.suggested_bank:
                            raise RuntimeError(
                                "Variable `{}` is in register `{}`, which is not in suggested bank {}.".format(
                                    dst_var.name, dst_var.register.name, dst_var.suggested_bank
                                )
                            )

    assert retval == 0 or (
        retval_instr is not None and __canScheduleInBundle(retval_instr, simulation)
    )  # We should always be able to schedule preparation instructions
    ...
    if retval == 0:
        ...
        if hasBankWriteConflict(retval_instr, simulation):
            ...
            # Write cycle conflict found, so, update found instruction cycle ready
            new_cycle_ready = CycleType(
                bundle=simulation.current_cycle.bundle, cycle=max(retval_instr.cycle_ready.cycle, simulation.current_cycle.cycle) + 1
            )
...
def scheduleASMISAInstructions(
    dependency_graph: nx.DiGraph, max_bundle_size: int, mem_model: MemoryModel, replacement_policy, progress_verbose: bool = False
) -> (list, list, list, int):
    """
        ...
        while simulation.dependency_graph:  # Iterates per instruction to be scheduled
            ...
            if new_bundle and len(simulation.xinsts) % simulation.max_bundles_per_xinstfetch == 0:
                if progress_verbose:
                    ...
                    print("{}% - {}/{}".format(pct, simulation.scheduled_xinsts_count, simulation.total_instructions))
                # Handle xinstfetch
                xinstfetch = cinst.XInstFetch(
                    len(simulation.xinstfetch_cinsts_buffer), simulation.xinstfetch_xq_addr, simulation.xinstfetch_hbm_addr
                )
            ...
            while True:  # do/while
                if (
                    simulation.topo_start_idx < len(simulation.full_topo_sort)
                    ...
                    if len(simulation.priority_queue) < Simulation.MIN_INSTRUCTIONS_IN_TOPO_SORT:
                        simulation.topo_sort += simulation.full_topo_sort[
                            simulation.topo_start_idx : simulation.topo_start_idx + Simulation.INSTRUCTION_WINDOW_SIZE
                        ]
                    ...
                    if fixed_last_short_bundle + 1 < simulation.current_cycle.bundle:
                        ...
                        if b_bundle_needs_fix:
                            ...
                            for bank_idx in range(1, len(simulation.mem_model.register_banks)):
                                mem_utilities.flushRegisterBank(
                                    simulation.mem_model.register_banks[bank_idx],
                            ...
                            for _, xinstr in simulation.priority_queue:
                                if xinstr.cycle_ready.bundle == simulation.current_cycle.bundle + 1:
                                    if xinstr.cycle_ready.cycle <= 1:
                                        xinstr.cycle_ready = CycleType(
                                            bundle=simulation.current_cycle.bundle, cycle=xinstr.cycle_ready.cycle
                                        )
                                        tmp_set.add(xinstr)
            ...
            if not b_flush_bundle:
                ...
                while xinstr is not None:
                    ...
                    if xinstr_prepped == 0:
                        ...
                        original_xinstr.cycle_ready = CycleType(bundle=simulation.current_cycle.bundle + 1, cycle=0)
                    ...
                    if xinstr:
                        ...
                        # Check if xinstruction is cycle ready for scheduling
                            ...
                            if xinstr:
                                ...
                                if scheduleXNOP(xinstr, idle_cycles_required, simulation):
                    ...
                    if xinstr:
                        ...
                        if original_xinstr == xinstr:
                            ...
                            for variable in original_xinstr.dests:
                                ...
                                assert variable.name not in simulation.mem_model.store_buffer or isinstance(original_xinstr, xinst.XStore)
                                if (
                                    variable.name in simulation.mem_model.output_variables
