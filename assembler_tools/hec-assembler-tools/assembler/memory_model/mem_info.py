...
class MemInfo:
    ...
    class Metadata:
        ...
        def parse_meta_field_from_mem_tokens(
            ...
            retval = None
            if len(tokens) >= 3:
                if tokens[0] == MemInfo.Const.Keyword.LOAD and tokens[1] == meta_field_name:
        ...
        def parse_from_mem_tokens(cls, tokens: list) -> MemInfoVariable:
            ...
            if len(tokens) >= 4:
                if tokens[0] == MemInfo.Const.Keyword.KEYGEN:
                    ...
                    var_name = tokens[3]
                    retval = MemInfoKeygenVariable(var_name=var_name, seed_index=seed_idx, key_index=key_idx)
            return retval
    ...
    class Input:
        ...
        def parse_from_mem_tokens(cls, tokens: list) -> MemInfoVariable:
            ...
            retval = None
            if len(tokens) >= 4:
                if tokens[0] == MemInfo.Const.Keyword.LOAD and tokens[1] == MemInfo.Const.Keyword.LOAD_INPUT:
                    hbm_addr = int(tokens[2])
                    var_name = tokens[3]
    ...
    def __init__(self, **kwargs):
        ...
        Clients should use MemInfo.from_file_iter() constructor to parse the contents of a .mem file.
        ...
        """
        self._keygens = [MemInfoKeygenVariable(**d) for d in kwargs.get(MemInfo.Const.FIELD_KEYGENS, [])]
        self._inputs = [MemInfoVariable(**d) for d in kwargs.get(MemInfo.Const.FIELD_INPUTS, [])]
        self._outputs = [MemInfoVariable(**d) for d in kwargs.get(MemInfo.Const.FIELD_OUTPUTS, [])]
        self._metadata = MemInfo.Metadata(**kwargs.get(MemInfo.Const.FIELD_METADATA, {}))
        self.validate()

    @property
    def factory_dict(self):
        """
    ...
    def get_meminfo_var_from_tokens(cls, tokens) -> tuple[Optional[MemInfoVariable], Optional[type]]:
        ...
        Returns:
            tuple[MemInfoVariable,type]: The parsed MemInfo variable and its type, or None if no variable could be parsed.
        """
        miv: Optional[MemInfoVariable] = None
        mem_info_type: Optional[type] = None
        for mem_info_type in cls.mem_info_types:
            ...
            if s_line:  # skip empty lines
                ...
                if tokens and len(tokens) > 0:
                    try:
                        ...
                        raise RuntimeError(f"{e} {line_no}: {s_line}") from e
        retval.validate()
        return retval

    @classmethod
    def from_dinstrs(cls, dinstrs):
        """
        Creates a new MemInfo object from an list of DInstructions.

        Args:
            dinstrs (list[DInstruction]): List of DInstructions.

        Raises:
            RuntimeError: If there is an error parsing the instruction tokens.
        ...
        retval = cls()
        for ints_no, dinstr in enumerate(dinstrs, 1):
            tokens = dinstr.tokens
            if tokens and len(tokens) > 0:
                try:
                    retval.add_meminfo_var_from_tokens(tokens)
                except RuntimeError as e:
    ...
    def validate(self):
        ...
        if len(self.metadata.ones) * MemoryModel.MAX_TWIDDLE_META_VARS_PER_SEGMENT != len(self.metadata.twiddle):
            raise RuntimeError(
                ("Expected {} times as many twiddles as ones metadata values, " "but received {} twiddles and {} ones.").format(
        ...
        for var_info in all_var_info:
            if var_info.var_name not in mem_info_vars:
                ...
                raise RuntimeError(
                    ('Variable "{}" already allocated in HBM address {}, ' "but new allocation requested into address {}.").format(
                        var_info.var_name,
