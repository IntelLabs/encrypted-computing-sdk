...
class SPAD(MemoryBank):
    ...
    class AccessTracker:
        ...
        def __init__(self, last_mload=None, last_mstore=None, last_cload=None, last_cstore=None):
    ...
    def __init__(self, data_capacity_words: int):
        ...
        if data_capacity_words > mmconstants.SPAD.MAX_CAPACITY_WORDS:
            raise ValueError(
                ("`data_capacity_words` must be in the range (0, {}], " "but {} received.").format(
                    mmconstants.SPAD.MAX_CAPACITY_WORDS, data_capacity_words
                )
            )
    ...
    def allocateForce(self, addr: int, variable: Variable):
        ...
        if variable.spad_address < 0:
            ...
            raise ValueError(
                (
                    '`variable` already allocated in address "{}", ' 'but new allocation requested in address "{}".'.format(
                        variable.spad_address, addr
                    )
                )
            )
    ...
    def findAvailableAddress(self, live_var_names, replacement_policy: str = None) -> int:
        """
        ...
        """
        return utilities.findAvailableLocation(self.buffer, live_var_names, replacement_policy)
