...
if __name__ == "__main__":
    ...
    if args.verbose > 0:
        print(module_name)
        print()
        print(f"P-ISA: {pisa_prep_file}")
        print(f"Xinst File: {xinst_file}")
        print(f"Output Name: {output_file}")
        print(f"Tracking: {variables_set}")
        ...
        with open(output_file, "w") as f_out:
            for line in f_in:
                ...
                if int(s_split[1]) in pisa_instr_num_set:
                    ...
                    out_line = f"{s_split[0]}, {s_split[1]}, nop, {s_split[7]} # rshuffle"
                elif "nop" in s_line:
                    # Keep nops timing
                    out_line = s_line.strip()
                if not out_line:
                    # Any other instructions are converted to single cycle nop
                    out_line = f"{s_split[0]}, {s_split[1]}, nop, 0"
